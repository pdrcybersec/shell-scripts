...
# Run traceroute
traceroute "${1}" | tee -a "${OUTPUT}"/"${1}"_"${2}"_"${FILENAME}"_"${NOW}".txt
...
echo ""
traceroute "${1}" | grep -E -o "([0-9]{1,3}[\\.]){3}[0-9]{1,3}" | sort -u | tee -a "${OUTPUT}"/"${1}"_"${2}"_"${FILENAME}"_"${NOW}".txt

echo ""
echo "OUTPUT FILE:" "${OUTPUT}"/"${1}"_"${2}"_"${FILENAME}"_"${NOW}".txt
echo ""
